...
export function bindFamilyPrototype(root:HTMLElement,render:()=>void,close:()=>void){
 ...
 for(const button of root.querySelectorAll<HTMLButtonElement>('[data-family-action]'))button.addEventListener('click',()=>{
  ...
  if(action==='submit'){
   ...
   const record={problemId:p.id,parameters:p.model.parameters,choiceId:selected,correct:choice.correct,mistakeType:choice.mistakeType,weaknessTags:choice.weaknessTags,crossSkills:choice.crossSkills,hypothesisOnly:true,at:new Date().toISOString(),generatorVersion:1};
