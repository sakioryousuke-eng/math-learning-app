import {tratioStrictEvidence} from '../tratio-bank/selector.ts';
...
export class LearningApplication {
 ...
 get snapshot(){
  ...
  const maxHistory=Object.entries(p.maxAttempts).flatMap(([unitId,attempts])=>attempts.map(a=>{if(unitId==='TRATIO'&&this.catalog.tratioSingleMax){const evidence=tratioStrictEvidence(this.catalog,p,a.at);return {unitId,challengeDate:a.at,problemId:a.problemId,success:evidence.some(e=>e.problemId===a.problemId&&e.at===a.at),independentSuccessCount:Math.min(1,evidence.length),acquiredAt:evidence[0]?.at??null};}const evidence=maxEvidence(this.master,p,unitId,a.at);return {unitId,challengeDate:a.at,problemId:a.problemId,success:a.noHint&&a.noMethodSpecified&&a.independent&&a.examQuality&&a.practicalTime&&a.correctConclusion&&a.readable&&a.sufficientWriting,independentSuccessCount:evidence.length,acquiredAt:evidence.length>=3?a.at:null};}));
  return {student:d.student,sessions:d.sessions,repairs:d.repairs,maxHistory,humanExp:{totalExp:p.humanExp,level:1+Math.floor(p.humanExp/5),events:d.expEvents},rank:mathematicsRank(p),map:this.master.units.map(u=>({unitId:u.id,status:unitStatus(this.master,p,u.id),requiredMax:u.prerequisites.filter(id=>!p.maxUnits.includes(id))})),evidence:p.assessments.map(a=>({assessmentId:a.id,skillId:a.skillId,problemId:a.problemId,result:a.solved?'correct':'unresolved',errorTags:a.tags,timestamp:a.at,evidenceType:a.context,isMaxEvidence:this.catalog.tratioSingleMax&&this.master.skills.find(s=>s.id===a.skillId)?.unitId==='TRATIO'?a.context==='max'&&tratioStrictEvidence(this.catalog,p,a.at).some(e=>e.problemId===a.problemId&&e.at===a.at):a.context==='max'&&Object.keys(p.maxAttempts).some(unit=>maxEvidence(this.master,p,unit,a.at).some(e=>e.problemId===a.problemId&&e.at===a.at))||Object.values(p.diagnosticMaxEvidence).flat().some(e=>e.assessmentId===a.id)})),diagnostic:{completed:p.diagnosticCompleted,passed:d.checkpoint.passed,pending:p.diagnosticPending,evidence:p.assessments.filter(a=>a.context==='diagnostic')},revision:d.revision,namespace:d.namespace};
 }
 ...
 async useReviewedPreview(id:string,stable=false){if(!this.development)throw new PersistenceError('MODE','開発モード専用です。');await this.exclusive(()=>this.open(`demo:preview:${stable?'after':'before'}:${id}`,()=>this.service.openReviewedPreview(id,stable),true));}
 ...
 async resetDemo(){if(!this.development||!this.namespace.startsWith('demo:'))throw new PersistenceError('MODE','通常学習はこの操作では初期化できません。');const key=this.namespace;await this.exclusive(()=>this.open(key,()=>{if(key.startsWith('demo:preview:')){const parts=key.split(':');this.service.openReviewedPreview(parts.slice(3).join(':'),parts[2]==='after');}else if(key.startsWith('demo:verify:'))seedVerification(this.service,this.catalog,key.slice('demo:verify:'.length) as VerificationState);else if(key.startsWith('demo:grading:'))this.service.openGradingDemo(key.slice('demo:grading:'.length));else this.service.switchProfile(key.slice(5) as Profile);},true));}
}
