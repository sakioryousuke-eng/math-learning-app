...
import {master,curriculum} from '../curriculum/master.ts';
...
function render(){
 ...
 const stableCount=master.skills.filter(x=>x.unitId===p.activeUnit&&p.skills[x.id]==='stable').length;
 const unitSkills=master.skills.filter(x=>x.unitId===p.activeUnit).length;
 ...
 if(s.screen==='home'){
  ...
  content=`<div class="eyebrow">MAX CHALLENGE / 実戦確認</div><h1>${esc(unitName(s.unitId))}</h1><section class="max-card"><span class="max-mark">☆ MAX</span><h2>方法を選び、最後まで。</h2><p class="muted">第5段階の各単元に独立したMAX代表問題を用意しています。入試水準・所要時間の校正は未完了です。</p><div class="evidence-circles" aria-label="独立成功 ${count} 回">${[1,2,3].map(i=>`<span class="${i<=count?'filled':''}">${i<=count?'✓':i}</span>`).join('')}</div><p><strong>独立成功 ${count} / 3</strong></p><p class="muted">証拠の期限：${esc(expired)}</p></section><ul class="conditions"><li>ノーヒント・方法指定なし</li><li>根拠と最終結論を含む答案</li><li>独立した問題で3回の成功</li><li>原則1日1回。失敗してもstableは保持</li></ul>${s.usedToday?'<div class="notice">本日の挑戦は提出済みです。次の挑戦は翌日です。</div>':''}${!s.availableMax&&s.unitId?'<div class="notice">この単元の固定MAX代表問題は使用済みです。独立した別問題の追加が必要です。</div>':''}${button('MAX問題に挑戦','start-max',false,!s.unitId||!!s.usedToday||!s.availableMax||!!s.current&&!s.result)}<p class="muted">${devMode?'開発用の仮採点で確認します。時計は上部の「開発確認」から進められます。':'実答案の採点は次段階で接続します。'}</p>`;
 }
 app.innerHTML=`<header class="site-header"><div class="brand"><span class="brand-icon">∑</span><span>数学の道<small>第5段階 · 数学全体マップ</small></span></div><span class="header-badge">${learning.namespace.startsWith('demo:')?'DEMO':'LOCAL'}</span></header><div class="shell">${dev}<p class="status-message" role="status">${saving?'保存中…':'端末に保存済み'} · ${learning.namespace.startsWith('demo:')?'開発デモ':'通常学習'}</p><main id="main" tabindex="-1">${error?`<div class="error" role="alert">${esc(error)}</div>`:''}${s.notice?`<p class="status-message" role="status">${esc(s.notice)}</p>`:''}${content}</main></div><nav class="bottom-nav" aria-label="メインナビ"><button data-action="home" ${s.screen==='home'||s.screen==='diagnostic'?'aria-current="page"':''}><span>⌂</span>ホーム</button><button data-action="map" ${s.screen==='map'?'aria-current="page"':''}><span>⋮</span>攻略マップ</button><button data-action="records" ${s.screen==='records'?'aria-current="page"':''}><span>▤</span>記録</button></nav>`;
...
app.addEventListener('click',e=>{
 ...
 switch(action){
   ...
   case'submit':{const value=app.querySelector<HTMLInputElement>('input[name="outcome"]:checked')?.value;if(!value)throw new Error('結果を選んでください。');service.submit(value as MockOutcome);break;}
