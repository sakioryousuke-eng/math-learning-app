...
for(const p of countProblems)test(`count family ${p.index}: equations, discriminant, parameter sets, intersections and paired graphs`,()=>{
 ...
 const spec=countDynamicSpec(p);assert.deepEqual(spec.boundaries,m.boundaries.map(b=>({value:b.value,label:b.text})));for(const b of m.boundaries){assert.equal(spec.region(b.value)%2,1);assert.ok(spec.reason(b.value).includes('1個'));assert.deepEqual(spec.graph(b.value),countGraphState(z,b.value).original);assert.deepEqual(spec.comparisonGraph!(b.value),countGraphState(z,b.value).difference);}
